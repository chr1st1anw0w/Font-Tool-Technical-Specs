...
import { ParametersIcon, SparklesIcon, PenToolIcon, ColorSwatchIcon, BevelIcon, NoBevelIcon, ChamferIcon, FilletIcon, InsetSquareIcon, InsetChamferIcon, InsetRoundIcon, ListIcon, RefreshIcon } from './icons';
...
const NodeBevelOverridePanel: React.FC<Pick<ControlPanelProps, 'selectedSegmentIds' | 'nodeOverrides' | 'onNodeOverrideChange' | 'onResetNodeOverrides'>> = ({
    selectedSegmentIds,
    nodeOverrides,
    ...
    const currentBevelType = firstOverride?.bevelType;
    const currentBevelSize = firstOverride?.bevelSize;

    return (
        <div className="space-y-4 bg-blue-50 border border-blue-200 p-3 rounded-lg">
             <div className="flex justify-between items-center">
                <p className="text-sm font-semibold text-blue-800">{selectedSegmentIds.length} 個節點的覆蓋設定</p>
                <button onClick={() => onResetNodeOverrides(selectedSegmentIds)} className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 font-medium" title="重設為全域設定">
            ...
            <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                    {bevelTypes.map(({ key, label, icon: Icon }) => (
                        <button
                            key={key}
                            onClick={() => onNodeOverrideChange(selectedSegmentIds, { bevelType: key })}
                            title={`設定倒角類型為：${label}`}
                            className={clsx(
            ...
            <Slider
                ...
                max={50}
                step={0.5}
                disabled={currentBevelType === BevelType.NONE}
            />
        </div>
    );
};
