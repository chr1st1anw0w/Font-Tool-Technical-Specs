...
class AIService {
    ...
    async textToSVG(prompt: string): Promise<string> {
        if (!this.ai) {
            throw new Error("AI 服務未初始化。請設定您的 Gemini API 金鑰。");
        }

        const fullPrompt = `
            Generate a single, clean, black-filled SVG for the following request.
            The response must contain ONLY the SVG code and nothing else.
            - The SVG MUST have a viewBox attribute, for example: viewBox="0 0 500 500".
    ...
    async getStyleSuggestions(prompt: string): Promise<TransformParams[]> {
        if (!this.ai) {
            throw new Error("AI 服務未初始化。請設定您的 Gemini API 金鑰。");
        }
        ...
        const schema = {
            ...
            items: {
                ...
                properties: {
                    weight: {
                        type: Type.INTEGER,
                        description: 'Font weight (thickness). Range: 1 to 200.',
                    },
                    width: {
                        type: Type.INTEGER,
                        description: 'Font width percentage. Range: 50 to 150.',
                    },
                    slant: {
                        type: Type.INTEGER,
                        description: 'Font slant in degrees. Range: -30 to 30.',
                    },
                },
        ...
        try {
            ...
            if (!Array.isArray(suggestions) || suggestions.length === 0) {
                throw new Error("AI 未能返回有效的樣式建議。");
            }
            
            // Basic validation
            return suggestions.filter(s =>
                s.hasOwnProperty('weight') &&
                s.hasOwnProperty('width') &&
                s.hasOwnProperty('slant')
            );
